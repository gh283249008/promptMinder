...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const scheduleNext = () => {
      ...
      timerRef.current = setTimeout(() => { loadUnreadCount(); }, delay);
    };

    const loadUnreadCount = async ({ force = false } = {}) => {
      if (!active) return;
      if (typeof document !== 'undefined' && document.visibilityState === 'hidden') { scheduleNext(); return; }
      if (typeof navigator !== 'undefined' && navigator.onLine === false) { scheduleNext(); return; }
    ...
    if (!disableBackgroundPolling) loadUnreadCount();

    const handleFocus = () => loadUnreadCount();
    const handleVisibilityChange = () => { if (document.visibilityState === 'visible') loadUnreadCount(); };
    const handleOnline = () => loadUnreadCount({ force: true });
  ...
  const fallbackTranslations = {
    header: { manage: '控制台', public: '合集' },
    navbar: { manage: '提示词管理', new: '新建提示词', public: '公共库', menuTitle: '菜单', menuSubtitle: '导航菜单', teamSection: '团队', navigationSection: '导航', settingsSection: '设置' },
    language: { switchTo: '切换语言', current: '中文' },
  };

  const translations = t || fallbackTranslations;

  const navItems = [
    { href: '/prompts', label: translations.header?.manage || translations.navbar.manage, icon: Library },
    { href: '/playground', label: translations.header?.playground || 'Playground', icon: FlaskConical },
    { href: '/public', label: translations.header?.public || translations.navbar.public, icon: LayoutGrid },
  ];
    ...
    <nav className="border-b border-white/20 bg-white/70 backdrop-blur-xl shadow-sm">
      <div className="mx-auto flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex items-center gap-2">
          <OptimizedImage src="/logo2.png" alt="PromptMinder" width={40} height={40} priority className="rounded-xl" />
        ...
        <div className="flex items-center gap-3 sm:gap-6">
            ...
            <NavigationMenuList className="space-x-2">
              {navItems.map(({ href, label, icon: Icon }) => (
                <NavigationMenuItem key={href}>
                  <NavigationMenuLink
                    asChild
                    className={`${pathname === href ? 'bg-slate-100 text-slate-900' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'} flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors`}
                  >
          ...
          <Sheet>
            ...
            <SheetContent side="right" className="w-[280px] sm:w-[320px]">
              <SheetTitle className="sr-only">{translations.navbar.menuTitle}</SheetTitle>
              <div className="flex flex-col gap-1 pb-4 border-b">
                <p className="text-xs text-muted-foreground">{translations.navbar.menuSubtitle}</p>
              </div>
              <div className="flex flex-col gap-6 mt-6">
                {showTeamSwitcher && (
                  <>
                    <div className="space-y-2">
                      <div className="text-xs font-medium text-muted-foreground px-2">{translations.navbar.teamSection}</div>
                ...
                <nav className="space-y-1">
                  <div className="text-xs font-medium text-muted-foreground px-2 mb-2">{translations.navbar.navigationSection}</div>
                  {navItems.map(({ href, label, icon: Icon }) => (
                    <Link
                      key={href}
                      href={href}
                      className={`${pathname === href ? 'bg-primary/10 text-primary font-medium border-l-2 border-primary' : 'text-muted-foreground hover:bg-accent hover:text-accent-foreground'} flex items-center gap-3 px-3 py-2.5 rounded-md transition-all duration-200`}
                    >
                ...
                <div className="space-y-1">
                  <div className="text-xs font-medium text-muted-foreground px-2 mb-2">{translations.navbar.settingsSection}</div>
                  <Link href="/settings/cli-tokens" className="flex items-center gap-3 rounded-md px-3 py-2.5 text-sm text-muted-foreground transition-all duration-200 hover:bg-accent hover:text-accent-foreground">
                    <Terminal className="h-4 w-4 shrink-0" />
                    <span>CLI Tokens</span>
                  </Link>
                  <Button variant="ghost" onClick={toggleLanguage} className="w-full justify-start gap-3 px-3 py-2.5 h-auto text-muted-foreground hover:bg-accent hover:text-accent-foreground">
